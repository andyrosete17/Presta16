...
var AppOPC = {
    ...
    registerEvents: function(){
        ...
        $('button#btn-expand-collapse-all').on('click', function() {
            $('#content_translations .panel-title .accordion-toggle').trigger('click');
        });
        ...
        $('div.tab-content div#tab-suggestions a[id="suggestions-contact"]').attr({'target': '_blank', 'href': 'https://addons.prestashop.com/en/write-to-developper?id_product=8503'});
        $('div.tab-content div#tab-suggestions a[id="suggestions-opinions"]').attr({'target': '_blank', 'href': 'http://addons.prestashop.com/ratings.php'});
    ...
    toggleIconCollapse: function(e) {
        ...
        .prev('.panel-heading')
        .find("i.indicator")
        .toggleClass('fa-pts-minus fa-plus');
    },
    shareTranslation: function(){
        var data = {
            action: 'shareTranslation',
            iso_code: $('div.tab-content div#tab-translate select#lst-id_lang').val()
        };
...
//        }
    ...
    edit: function(event, data) {
        ...
        if (parseInt(data.is_custom)) {
            ...
            if (data.name == 'id_country' || data.name == 'id_state' || data.name == 'alias' || data.name == 'newsletter') {
                $('#form_required_fields #txt-field_default_value').prop('disabled', false).closest('.form-group').show();
