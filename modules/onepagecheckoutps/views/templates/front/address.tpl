...
{*remove comments in case you do not want addresses in tabs - just remove asterisks*}
{*assign var='addresses_tab' value=0*}
...
{if isset($OPC_FIELDS[$OPC_GLOBALS->object->customer])}
	...
	{if !$isLogged && isset($opc_social_networks) && ($opc_social_networks->facebook->client_id neq '' || $opc_social_networks->google->client_id neq '' || $opc_social_networks->paypal->client_id neq '')}
        ...
        {if ($CONFIGS.OPC_SHOW_DELIVERY_VIRTUAL && $IS_VIRTUAL_CART) or !$IS_VIRTUAL_CART}
            ...
            <section id="delivery_address_container" class="{if $addresses_tab}page-product-box tab-pane active{/if}">
                ...
                {if $CONFIGS.OPC_ENABLE_INVOICE_ADDRESS}
                    <div class="row">
                        <div class="form-group col-xs-12 container_help_invoice">
