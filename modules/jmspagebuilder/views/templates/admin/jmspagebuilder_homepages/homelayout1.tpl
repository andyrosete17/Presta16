...
<div class="layout-tools">		
		<select id="select-home">
			{foreach from=$homepages key=i item=home}
				<option value="{$home.id_homepage|escape:'htmlall':'UTF-8'}" {if $home.id_homepage == $homepage.id_homepage|escape:'htmlall':'UTF-8'}selected="selected"{/if}>{$home.title|escape:'htmlall':'UTF-8'}</option>
			{/foreach}
		</select>
		...
		<ul class="import-export">
			...
			<li><a class="export-link label-tooltip" data-original-title="Export Layout" href="{$link->getAdminLink('AdminJmspagebuilderHomepages')|escape:'htmlall':'UTF-8'}&configure=jmspagebuilder&export_id_homepage={$homepage.id_homepage|escape:'htmlall':'UTF-8'}"><i class="icon-download"></i></a></li>			
...
<form name="adminForm" action="{$link->getAdminLink('AdminJmspagebuilderHomepages')|escape:'htmlall':'UTF-8'}&configure=jmspagebuilder" method="post" enctype="multipart/form-data">
		<select name="import_file">
			{foreach from=$jsonfiles key=i item=jsonfile}
			<option value="{$jsonfile|escape:'htmlall':'UTF-8'}">{$jsonfile|escape:'htmlall':'UTF-8'}</option> 	
			{/foreach}
		</select>
		<button type="submit">Import</button>			
		<br /><span>{l s='Files in' mod='jmspagebuilder'} <strong>modules/jmspagebuilder/json/</strong> Folder</span>
		<input type="hidden" name="import_id_homepage" value="{$homepage.id_homepage|escape:'htmlall':'UTF-8'}" /> 
</form>	
</div>
<div class="language-form" style="display:none;">
<form name="adminForm" action="{$link->getAdminLink('AdminJmspagebuilderHomepages')|escape:'htmlall':'UTF-8'}&configure=jmspagebuilder" method="post">
	...
	<input type="hidden" name="lang_id_homepage" value="{$homepage.id_homepage|escape:'htmlall':'UTF-8'}" /> 
</form>
</div>
<div class="pagebuilder-panel">
	<div id="rowlist" class="lg-layout" data-id="{$homepage.id_homepage|escape:'htmlall':'UTF-8'}">
	<div class="rowlist">
		{foreach from=$rows key=i item=row}
			<div class="row" data-layout="{$row->layout|escape:'htmlall':'UTF-8'}" data-name="{if isset($row->name)}{$row->name|escape:'htmlall':'UTF-8'}{/if}"  data-custom_class="{if isset($row->settings->custom_class)}{$row->settings->custom_class|escape:'htmlall':'UTF-8'}{/if}" data-fluid="{$row->settings->fluid|escape:'htmlall':'UTF-8'}" data-background_img="{if isset($row->settings->background_img)}{$row->settings->background_img|escape:'htmlall':'UTF-8'}{/if}" data-background_size="{if isset($row->settings->background_size)}{$row->settings->background_size|escape:'htmlall':'UTF-8'}{/if}" data-background_repeat="{if isset($row->settings->background_repeat)}{$row->settings->background_repeat|escape:'htmlall':'UTF-8'}{/if}" data-background_position="{if isset($row->settings->background_position)}{$row->settings->background_position|escape:'htmlall':'UTF-8'}{/if}" data-background_attachment="{if isset($row->settings->background_attachment)}{$row->settings->background_attachment|escape:'htmlall':'UTF-8'}{/if}" data-hook="{if isset($row->settings->hook)}{$row->settings->hook|escape:'htmlall':'UTF-8'}{/if}">
				<div class="row-title">						
					<div class="pull-left">
						<span><i class="icon-arrows"></i></span>						
						<strong class="row-name">{if isset($row->name)}{$row->name|escape:'htmlall':'UTF-8'}{/if}</strong>
					</div>
					<div class="pull-right">
						<ul class="button-group" role="menu" aria-labelledby="dLabel">
							...
							<li>
								<a class="btn btn-default remove-row label-tooltip" data-original-title="Delete This Row"><i class="icon-remove"></i></a>
							</li>	
						</ul>
					</div>
				</div>	
				<div class="row-columns">
					{foreach from=$row->cols key=cindex item=column}
						<div class="{$column->className|escape:'htmlall':'UTF-8'}" data-custom_class="{if isset($column->settings->custom_class)}{$column->settings->custom_class|escape:'htmlall':'UTF-8'}{/if}" data-md_col="{if isset($column->settings->md_col)}{$column->settings->md_col|escape:'htmlall':'UTF-8'}{/if}" data-sm_col="{if isset($column->settings->sm_col)}{$column->settings->sm_col|escape:'htmlall':'UTF-8'}{/if}" data-xs_col="{if isset($column->settings->xs_col)}{$column->settings->xs_col|escape:'htmlall':'UTF-8'}{/if}" data-background_img="{if isset($column->settings->background_img)}{$column->settings->background_img|escape:'htmlall':'UTF-8'}{/if}" data-background_size="{if isset($column->settings->background_size)}{$column->settings->background_size|escape:'htmlall':'UTF-8'}{/if}" data-background_repeat="{if isset($column->settings->background_repeat)}{$column->settings->background_repeat|escape:'htmlall':'UTF-8'}{/if}" data-background_position="{if isset($column->settings->background_position)}{$column->settings->background_position|escape:'htmlall':'UTF-8'}{/if}" data-background_attachment="{if isset($column->settings->background_attachment)}{$column->settings->background_attachment|escape:'htmlall':'UTF-8'}{/if}">
							<div class="column">
								{foreach from=$column->addons key=aindex item=addon}
									{$addon->addon_box|escape:'htmlall':'UTF-8'}
								{/foreach}
							</div>
							<div class="col-tools">
...
<input id="root_url" type="hidden" name="root_url" value="{$root_url|escape:'htmlall':'UTF-8'}" />
<input id="ajax_url" type="hidden" name="ajax_url" value="{$ajax_url|escape:'htmlall':'UTF-8'}" />
<input id="current_url" type="hidden" name="current_url" value="{$link->getAdminLink('AdminJmspagebuilderHomepages')|escape:'htmlall':'UTF-8'}&configure=jmspagebuilder&editAddon=1" />
<input id="backend_url" type="hidden" name="backend_url" value="{$link->getAdminLink('AdminJmspagebuilderHomepages')|escape:'htmlall':'UTF-8'}" />
<div class="hidden">
...
<div class="hidden">
	<div class="addon-box module" data-addon="module" data-modulename="" data-hook="">
		...
		<div class="addon-tools">
			...
			<a class="remove-addon"><i class="icon-remove"></i></a>				
		</div>
	</div>	
</div>
<div class="hidden">
	<div class="row-settings">
		<div class="form-group">
			<label>{l s='Row Name' mod='jmspagebuilder'}</label>
			<input type="text" value="" data-attrname="name" class="form-control addon-input addon-name">
		</div>
		<div class="form-group">
			<label>{l s='Hook' mod='jmspagebuilder'}</label>
		...
		<div class="form-group">
			<label>{l s='Custom CSS Class' mod='jmspagebuilder'}</label>
		...
		<div class="form-group">
			<div class="checkbox"><label><input type="checkbox" data-attrname="fluid" class="addon-input input-fluid">{l s='Fluid Width' mod='jmspagebuilder'}</label></div>
			<p class="help-block">Enable this option to make this row fluid. Fluid row will help you to publish full width content.</p>
		</div>
		<div class="form-group">
			<label>{l s='Background Image' mod='jmspagebuilder'}</label>
		...
		<div class="form-group">
			<label>{l s='Background Size' mod='jmspagebuilder'}</label>
		...
		<div class="form-group">
			<label>{l s='Background Repeat' mod='jmspagebuilder'}</label>
		...
		<div class="form-group">
			<label>{l s='Background Position' mod='jmspagebuilder'}</label>
		...
		<div class="form-group">
			<label>{l s='Background Attachment' mod='jmspagebuilder'}</label>
...
<div class="hidden">
	<div class="column-settings">
		...
		<div class="form-group md_col">
			<label>{l s='Medium Layout' mod='jmspagebuilder'}</label>
		...
		<div class="form-group sm_col">
			<label>{l s='Tablet Layout' mod='jmspagebuilder'}</label>
		...
		<div class="form-group xs_col">
			<label>{l s='Mobile Layout' mod='jmspagebuilder'}</label>
	...
	<ul class="pagebuilder-addons clearfix">	
		{foreach from=$addonslist key=i item=addonlist}
			<li class="addon-cat-addons">
				{$addonlist|escape:'htmlall':'UTF-8'}					
			</li>
		{/foreach}
		{foreach from=$modules key=i item=module}
			<li class="addon-cat-modules">
				<a class="addon-title" data-module="{$module.name|escape:'htmlall':'UTF-8'}">			
					<img src="{$modules_url|escape:'htmlall':'UTF-8'}/{$module.name|escape:'htmlall':'UTF-8'}/logo.png" />
					<span class="element-title">{$module.name|escape:'htmlall':'UTF-8'}</span>
					<span class="element-description">{if isset($module.short_desc)}{$module.short_desc|escape:'htmlall':'UTF-8'}{/if}</span>
				</a>
			</li>
		{/foreach}
	</ul>	
    ...
    <div class="jms-modal-dialog">
        <div class="jms-modal-content">
            ...
            <div class="jms-modal-footer clearfix">
                <a href="javascript:void(0)" class="btn btn-success pull-left" id="save-settings" data-dismiss="modal">{l s='Apply' mod='jmspagebuilder'}</a>
                <button class="btn btn-danger pull-left" data-dismiss="modal" aria-hidden="true">{l s='Cancel' mod='jmspagebuilder'}</button>
            </div>
        </div>
    </div>
</div>
<div class="jms-modal" id="addon-modal" tabindex="-1" role="dialog" aria-labelledby="modal-label" aria-hidden="true" style="display: none;">
    <div class="jms-modal-dialog">
        <div class="jms-modal-content">
            ...
            <div class="jms-modal-body"></div>
            <div class="jms-modal-footer clearfix">
                <a href="javascript:void(0)" class="btn btn-success pull-left" id="save-addons" data-dismiss="modal">{l s='Apply' mod='jmspagebuilder'}</a>
                <button class="btn btn-danger pull-left" data-dismiss="modal" aria-hidden="true">{l s='Cancel' mod='jmspagebuilder'}</button>
            </div>
        </div>
    </div>
...
<div class="jms-modal fade" id="modal-addons" tabindex="-1" role="dialog" aria-labelledby="modal-addon-label" aria-hidden="true">
	<div class="jms-modal-dialog jms-modal-xlg">
		<div class="jms-modal-content">
			<div class="jms-modal-header">
				<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
				<h3 class="jms-modal-title" id="modal-addons-label">{l s='Add Addon or Module' mod='jmspagebuilder'}</h3>
...
<input type="hidden" name="mediatoken" id="mediatoken" value="{$mediatoken|escape:'htmlall':'UTF-8'}" />
<form name="layoutForm" action="{$link->getAdminLink('AdminJmspagebuilderHomepages')|escape:'htmlall':'UTF-8'}&configure=jmspagebuilder" method="post">
<input type="hidden" name="jmsformjson" id="jmsformjson" value="" />
<input type="hidden" name="json_home_id" value="{$homepage.id_homepage|escape:'htmlall':'UTF-8'}" />
	...
	$(document).ready(function(){
		{block name="autoload_tinyMCE"}
			tinySetup({
				...
				document_base_url : "{$root_url|escape:'htmlall':'UTF-8'}"
			});
		{/block}
	});	
	$(document).ready(function(){
