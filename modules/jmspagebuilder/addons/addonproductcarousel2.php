...
class JmsAddonProductCarousel2 extends JmsAddonBase
{
    public function __construct()
    {
        $this->addonname = 'productcarousel2';
        $this->modulename = 'jmspagebuilder';
        $this->addontitle = 'Product Carousel 2';
    ...
    public function getProducts($fields)
    {
        ...
        $total_config = $fields[4]->value;
        $rows = $fields[5]->value;
        $cols = $fields[6]->value;
        $category_ids = array();
        if ($fields[10]->value) {
            $category_ids = explode(',', $fields[10]->value);
        }
        $_products = array();
    ...
    public function returnValue($addon)
    {
        ...
        $category_ids = explode(",", $addon->fields[10]->value);
        $k=0;
        foreach ($category_ids as $id_category) {
            $category = new Category($id_category, (int)Context::getContext()->language->id);
            $list_categories[$k]['id_category'] = $id_category;
            $list_categories[$k]['name'] = $category->name;
            $list_categories[$k]['link_rewrite']=$category->link_rewrite;
            $k++;
        }
        ...
        $this->context->smarty->assign(
            array(
                ...
                'addon_title' => $addon->fields[1]->value->$id_lang,
                'addon_desc' => $addon->fields[2]->value->$id_lang,
                'producttype' => $addon->fields[3]->value,
                ...
                'banner_html' => JmsPageBuilderHelper::decodeHTML($addon->fields[16]->value->$id_lang),
                'banner_link' => $addon->fields[17]->value,
