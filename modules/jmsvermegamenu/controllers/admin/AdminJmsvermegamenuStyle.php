...
class AdminJmsvermegamenuStyleController extends ModuleAdminController
{
    public function __construct()
    {
        ...
        $this->menu = '';
        $this->children = array();
        $this->url_root = JmsVerMegaMenuHelper::getUrl();
        $this->_items = array();
        $this->gens = array();
    ...
    private function _postProcess()
    {
        ...
        if (count($errors)) {
            ...
            Tools::redirectAdmin($this->context->link->getAdminLink('AdminJmsvermegamenuStyle', true).'&conf=4&configure='.$this->name.'&tab_module='.$this->tab.'&module_name='.$this->name);
        }
    }
    ...
    {
        if (Tools::getValue('controller') != 'AdminJmsvermegamenuStyle' && Tools::getValue('configure') != $this->name) {
            return;
        }
            ...
            foreach ($rows as $row) {
                ...
                foreach ($row as $col) {
                    ...
                    foreach ($col->items as $_item) {
                        if (isset($this->_items[$_item->item]) && (int)$this->_items[$_item->item]['parent_id'] == $itemid) {
                            $col_show++;
                        }
                    }
